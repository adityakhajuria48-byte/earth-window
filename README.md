...
## Resolution filters and Python area previews · 18 September 2026

- **Refine your search → Spatial resolution:** all resolutions, 10 m or finer, or 30 m or finer. A result needs at least one reported band / nominal pixel spacing within the limit. Unknown values are excluded only when a limit is active. This is a spatial-resolution filter, not a spectral-resolution filter. It filters loaded results and never skips a connected archive; other bands in a matching product may be coarser.
- **Public INPE band previews:** the Python service reads a bounded native crop from the provider and returns a masked source-pixel array. The browser applies the existing 2–98% display stretch. Single bands and available RGB combinations are supported; authenticated provider files remain unavailable without provider access. This supersedes earlier notes that all INPE bands are download-only.
- **Preview area & GeoTIFF export:** change west/south/east/north bounds here. Point searches start with a 0.02° box; area searches use the searched area; worldwide scene inspection starts around its bounding-box centre. The existing limit of 2° per side, 4 million native pixels, 64 MiB transferred / source-block memory, and 90 seconds per band remains. A smaller requested area cannot fix every oversized source block. Errors remain explicit; no reference mosaic is substituted.
- **Show preview on map:** place a successfully rendered single-band or RGB preview on the 2D map, or on the 3D globe when available. Before/after slots also accept supported server-assisted bands. Preview arrays are at most 512 pixels per side, with nearest-neighbour sampling and source masks; original GeoTIFF crop exports remain native resolution. No cloud or scientific quality mask is applied.
- `/api/preview` shares crop request validation, allowlisted source resolution, redirect rejection, authentication, transfer limits, subprocess timeout and one-job concurrency. The private Sites gateway forwards service credentials only to the configured Python service. `previews.py` is included in the Python edition and Docker image.
