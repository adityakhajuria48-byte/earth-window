...
function renderDiscovery(){
  ...
  for(const [title,f,note] of suggestions){if(!f)continue;const button=text('button','','match-card');button.type='button';button.append(text('span',title,'eyebrow'),text('strong',platform(f)),text('span',title==='Least reported cloud'?cloudLabel(f):EW.timeOffset(f,state.target)),text('small',note+' · '+(EW.source(f).access==='account'?'Provider account required':EW.bands(f).some(b=>b.canPreview)?'Band preview supported':EW.bands(f).some(b=>!b.requiresAuth)?'Public downloads':'Metadata only')));button.onclick=()=>openScene(f);matches.append(button);}
}
...
function openScene(f){
  ...
  const dl=text('dl','','detail-grid');const entries=[['Satellite',platform(f)],['Data provider',`${source.agency} · ${source.region}`],[t.composite?'Product period (UTC)':'Captured (UTC)',`${dateLabel(t.start)} · ${itemTime(f)}`],[t.composite?'Temporal detail':'Distance from chosen time',t.composite?`${source.period} · not an exact-time capture`:t.dayOnly?'Acquisition day only; exact time unavailable':t.range?'Acquisition interval; time at your exact point is not determined':`${(EW.distance(f,state.target)/3600000).toFixed(1)} hours ${t.start<state.target?'before':'after'}`],['Access',source.access==='account'?'Catalogue searchable · provider account required for data':source.preview==='download'?'Public originals · browser raster preview unavailable':'Public assets where supplied'],['Archive coverage',source.coverage||'Varies with mission swaths and available archive holdings'],['Cloud cover',cloudLabel(f)],['Pixel spacing',resolution(f)],['Collection',f.collection],['Scene / product ID',f.id]];
  for(const [k,v] of entries){const div=text('div','');div.append(text('dt',k),text('dd',v));dl.append(div);}content.append(dl);
  const pinActions=text('div','','capture-actions');for(const [slot,title] of [['a','Use as Before A'],['b','Use as After B']]){const button=text('button',title);button.type='button';button.disabled=!EW.bands(f).some(b=>b.canPreview);button.title=button.disabled?'This record requires provider access or external GIS software.':'';button.onclick=()=>{$('scene-dialog').close();EWStudio.pin(f,slot);};pinActions.append(button);}content.append(pinActions);
