...
function openScene(f){
  ...
  const pinActions=text('div','','capture-actions');for(const [slot,title] of [['a','Use as Before A'],['b','Use as After B']]){const button=text('button',title);button.type='button';button.onclick=()=>{$('scene-dialog').close();EWStudio.pin(f,slot);};pinActions.append(button);}content.append(pinActions);
