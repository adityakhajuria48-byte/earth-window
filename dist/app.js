...
const state = {lat:32.916,lon:75.141,name:'Udhampur, India',placeText:'Udhampur, India',scenes:[],date:initialDate,target:0,request:0,geoRequest:0,selected:null,query:null,dirty:false,shown:30,sourceResults:[]};
...
function clearResults(message) {state.shown=30;$('load-more').hidden=true;$('source-status').replaceChildren();state.scenes=[];state.selected=null;$('count').textContent='—';$('export').disabled=true;$('scene-list').replaceChildren(empty('Ready for another perspective',message));if(footprint&&map){map.removeLayer(footprint);footprint=null;}}
...
async function fetchScenes(q,signal,onProgress){
  await loadSources();
  if(window.EARTH_WINDOW_PYTHON)return fetchJSON('/api/search?'+new URLSearchParams({lat:q.lat,lon:q.lon,date:q.date,time:q.time,window:q.days,cloud:q.cloud}),signal);
...
async function search(event){
  ...
  $('search-button').disabled=true;$('search-button').textContent='Checking all satellite archives…';$('search-notice').textContent='';$('result-summary').textContent='Finding which satellites have data for this place and date…';
  ...
  try{
    ...
    $('result-summary').textContent=`${state.scenes.length} image / data result${state.scenes.length===1?'':'s'} · ${dateLabel(q.start)} – ${dateLabel(q.end)}${hasFailure?' · Coverage check incomplete. See archive status below.':''}`;
    renderSourceStatus(state.sourceResults);renderScenes();
...
function openScene(f){
  ...
  content.append(text('p','Previews show the complete source tile. Your selected point is within the recorded footprint, but valid pixels or clear ground are not guaranteed. The map remains the labelled NASA overview or street map.','detail-note'));
