...
window.EarthBands = (() => {
  ...
  function panel(f){
    ...
    function change(){revision++;controller?.abort();canvas.hidden=true;save.hidden=true;status.textContent='';render.disabled=false;render.textContent='Render band preview';bandField.hidden=mode.value!=='single';const selected=chosen();render.disabled=selected.some(b=>!b.canPreview);info.textContent=selected.map(b=>[b.label,typeof b.wavelength==='number'?`${b.wavelength} µm`:null,b.unit?`unit: ${b.unit}`:null].filter(Boolean).join(' · ')).join(' / ');open.hidden=mode.value!=='single';open.disabled=selected.some(b=>b.requiresAuth);links.replaceChildren();if(mode.value!=='single'){selected.forEach((b,i)=>{const btn=el('button',`${['R','G','B'][i]}: ${b.label} ↗`);btn.type='button';btn.disabled=b.requiresAuth;btn.onclick=()=>openFile(b);links.append(btn);});}if(render.disabled)status.textContent=selected.some(b=>b.requiresAuth)?'Provider sign-in is required. Band names describe the catalogue files; pixels have not been loaded.':EW.source(f).preview==='download'?'Original files are public, but this provider does not allow browser raster previews. Download and open them in QGIS.':'This asset format requires geospatial software. Use the file link to access it.';}
