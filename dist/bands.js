...
window.EarthBands = (() => {
  ...
  function panel(f,query){
    ...
    const buttons=el('div','','band-actions'),render=el('button','Render band preview'),cancel=el('button','Cancel preview'),onMap=el('button','Show preview on map'),open=el('button','Open selected band file ↗'),save=el('button','Save preview PNG ↓');
    render.type=cancel.type=onMap.type=open.type=save.type='button';save.hidden=onMap.hidden=cancel.hidden=true;buttons.append(render,cancel,onMap,open,save);box.append(buttons);
    ...
    const cropActions=el('div','','band-actions'),downloadCrop=el('button','Download GeoTIFF crop ↓'),cancelCrop=el('button','Cancel download'),recipe=el('button','Save crop request ↓'),cropStatus=el('p','','detail-note');downloadCrop.type=cancelCrop.type=recipe.type='button';cancelCrop.hidden=true;cropStatus.setAttribute('role','status');cropActions.append(downloadCrop,cancelCrop,recipe);cropBody.append(cropActions,cropStatus);
    ...
    cancelCrop.onclick=()=>cropController?.abort();
    downloadCrop.onclick=async()=>{
      const active=new AbortController();cropController=active;const timer=setTimeout(()=>active.abort(),150000);
      try{const request=cropRequest();downloadCrop.disabled=true;cancelCrop.hidden=false;cropStatus.textContent='Reading original source pixels. The service may need a moment to wake up…';
        const response=await fetch('/api/crop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request),signal:active.signal});
        if(!response.ok){const failure=await response.json();throw Error(failure.error||'Crop could not be exported.');}
        const blob=await response.blob();if(active.signal.aborted)return;saveBlob(blob,'earth-window-crop.tif');cropStatus.textContent='GeoTIFF downloaded at native resolution. Stored values and available calibration are preserved. Clouds and product quality flags are not masked.';
      }catch(e){if(cropController===active)cropStatus.textContent=active.signal.aborted?'Download cancelled or timed out. The server may finish its bounded job before accepting another.':e.message;}
      finally{clearTimeout(timer);if(cropController===active){cropController=null;cancelCrop.hidden=true;const b=chosen()[0];downloadCrop.disabled=!window.EARTH_WINDOW_CROPS||mode.value!=='single'||!b.isTiff||b.requiresAuth;}}
    };
    ...
    const dialog=document.getElementById('scene-dialog');const close=()=>{revision++;controller?.abort();cropController?.abort();cache.clear();dialog.removeEventListener('close',close);};dialog.addEventListener('close',close);
