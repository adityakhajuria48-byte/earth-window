...
window.EarthBands = (() => {
  ...
  function panel(f,query){
    ...
    const crop=el('details','','crop-panel'),cropBody=el('div','','crop-body');crop.append(el('summary','Export a georeferenced crop'),cropBody);box.append(crop);
    ...
    let initial=null,extentLabel='Enter your crop coordinates.';
    ...
    downloadCrop.onclick=async()=>{try{const request=cropRequest();downloadCrop.disabled=true;cropStatus.textContent='Reading original source pixels. This may take up to 90 seconds…';const response=await fetch('/api/crop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request),signal:AbortSignal.timeout(100000)});if(!response.ok){const failure=await response.json();throw Error(failure.error||'Crop could not be exported.');}saveBlob(await response.blob(),'earth-window-crop.tif');cropStatus.textContent='GeoTIFF downloaded at native resolution. Stored values and available calibration are preserved. Clouds and product quality flags are not masked.';}catch(e){cropStatus.textContent=e.name==='TimeoutError'?'Crop timed out. Try a smaller area.':e.message;}finally{downloadCrop.disabled=!window.EARTH_WINDOW_CROPS;}};
