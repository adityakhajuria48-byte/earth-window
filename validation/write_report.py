"""Build the human-readable report from preserved observations, after npm test."""
import csv
import json
...
source = json.loads((ROOT / 'locations-source.json').read_text())
initial = [json.loads(line) for line in (ROOT / 'location-checks.jsonl').read_text().splitlines()]
rechecks = [json.loads(line) for line in (ROOT / 'rechecks/location-checks.jsonl').read_text().splitlines()]
by_id = {r['id']: r for r in rechecks}
latest = [by_id.get(r['id'], r) for r in initial]
assert len(initial) == 1000 and len({r['id'] for r in initial}) == 1000
recovered = sum(r['ok'] for r in rechecks)
final_ok = sum(r['ok'] for r in latest)
terrain_ok = sum(r['terrain']['ok'] for r in latest)
all_tiles = [json.loads(line) for path in (ROOT / 'tile-checks.jsonl', ROOT / 'rechecks/tile-checks.jsonl') for line in path.read_text().splitlines()]
attempts = [a for t in all_tiles for a in t['attempts']]
latest_summary = dict(summary, rechecked_locations=len(rechecks), recovered_locations=recovered,
                      latest_locations_all_checks_passed=final_ok, latest_terrain_passed=terrain_ok,
                      remaining_failed_locations=[r['id'] for r in latest if not r['ok']],
                      total_http_attempts_including_rechecks=len(attempts))
(ROOT / 'final-summary.json').write_text(json.dumps(latest_summary, indent=2) + '\n')

fields = ['id', 'name', 'country', 'lat', 'lon', 'initial_all_ok', 'latest_all_ok', 'rechecked',
          'zoom8_source_initial', 'zoom14_source_initial', 'zoom19_source_initial',
          'terrain_initial_ok', 'terrain_latest_ok', 'terrain_min_m_latest', 'terrain_max_m_latest']
with (ROOT / 'results-1000.csv').open('w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=fields)
    writer.writeheader()
    for row in sorted(initial, key=lambda r: r['id']):
        current = by_id.get(row['id'], row)
        result = {k: row[k] for k in ('id', 'name', 'country', 'lat', 'lon')}
        result.update(initial_all_ok=row['ok'], latest_all_ok=current['ok'], rechecked=row['id'] in by_id,
                      terrain_initial_ok=row['terrain']['ok'], terrain_latest_ok=current['terrain']['ok'],
                      terrain_min_m_latest=current['terrain'].get('min_elevation_m'),
                      terrain_max_m_latest=current['terrain'].get('max_elevation_m'))
        result.update({f"zoom{t['requested_zoom']}_source_initial": t['source_zoom'] for t in row['imagery']})
        writer.writerow(result)

zoom_rows = '\n'.join(f"| {z} | {r['passed']:,}/1,000 | {r['native']:,} | {r['parent_fallback']:,} |" for z, r in summary['zoom_results'].items())
failed_rows = '\n'.join(f"| {r['name']} | {r['country']} | {'Passed' if by_id.get(r['id'], r)['ok'] else 'Still failed'} |" for r in initial if not r['ok'])
report = f"""# Earth Window: 1,000-location validation

**22 September 2026 — actual network observations, not simulated imagery.**

All 1,000 selected locations were checked at three imagery zoom levels and one elevation level. **{summary['imagery_passed']:,}/3,000 imagery checks passed**, including {summary['imagery_parent_fallback']} checks that recovered real coarser imagery. Terrain passed at **{summary['terrain_passed']:,}/1,000 locations initially** and **{terrain_ok:,}/1,000 after the separately recorded recheck**. {final_ok:,}/1,000 locations passed all four checks at their latest observation.
...
- Cities were selected in rounds across country/territory codes, largest population first within each code; duplicate coordinates were excluded. Extent: {source['latitude_range'][0]}° to {source['latitude_range'][1]}° latitude and {source['longitude_range'][0]}° to {source['longitude_range'][1]}° longitude. Polar regions, most open ocean and many rural environments are not represented.
...
The first pass examined **{summary['unique_tiles']:,} unique imagery/elevation tile keys** through **{summary['http_attempts']:,} HTTP attempts**. It recorded {summary['http_404_attempts']} missing-tile HTTP 404 responses and {summary['http_429_attempts']} HTTP 429 responses. It downloaded {summary['downloaded_valid_tile_bytes'] / 1024 / 1024:.1f} MiB of valid tile data. These totals count tile requests, not unique locations; retries, shared tiles and ancestor requests are separate concepts.
...
The {len(rechecks)} initially failed locations were rechecked separately; {recovered} recovered. Failures were connection/read timeouts or HTTP 502 responses, not corrupted LERC data. A network/proxy/service error does not establish that the provider lacks terrain at that location. Three Guam locations shared one failed tile.

| Location | Code | Separate recheck |
| --- | --- | --- |
{failed_rows}

The CSV preserves both initial and latest status. The original JSONL remains unchanged. The follow-up is in `rechecks/`; total HTTP attempts including it: **{len(attempts):,}**.
...
GeoNames source: {source['source']} — **CC BY 4.0**, [GeoNames](https://www.geonames.org/). Archive SHA-256: `{source['source_sha256']}`. The four landmark coordinates were added explicitly.
